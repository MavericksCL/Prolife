angular.module('Recalcine.controllers', [])
	...
	.controller('TabDashActiveCtrl', function($rootScope, $q, $geolocation, $scope, $localStorage, $stateParams, $toast, $localization, $state, $cordovaInAppBrowser, $cordovaDevice, $timeout){
		...
		$toast.loading($localization.get("MESSAGE.GEOLOCALING"));
	...
	.controller('TabAlarmHomeCtrl', function($cordovaNetwork, $rootScope, $scope, $interval, $filter, $localStorage, $profile, $toast, $localization, $log, Recordatorio){
		...
		$scope.changeAlarms = function(n, a){
			...
			var todayInitFuture = new Date();
			var todayEndFuture = new Date();
			if(todayInit.getHours() >= 12 ){ /* Entre 12 y 00 */
				...
				/* Post 00 */
				todayInitFuture.setDate(todayInitFuture.getDate()+1);
				todayInitFuture.setHours(0);
				todayInitFuture.setMinutes(0);
				todayInitFuture.setSeconds(1);
				todayInitFuture.setMilliseconds(0);
				todayEndFuture.setDate(todayEndFuture.getDate()+1);
				todayEndFuture.setHours(11);
				todayEndFuture.setMinutes(59);
				todayEndFuture.setSeconds(59);
				todayEndFuture.setMilliseconds(0);
			}else{ /* Entre 00 y 12 */
				...
				//todayEnd.setDate(todayEnd.getDate()+1);
				/* Post 12 */
				todayInitFuture.setHours(12);
				todayInitFuture.setMinutes(0);
				todayInitFuture.setSeconds(1);
				todayInitFuture.setMilliseconds(0);
				todayEndFuture.setHours(23);
				todayEndFuture.setMinutes(59);
				todayEndFuture.setSeconds(59);
				todayEndFuture.setMilliseconds(0);
			}
			...
			var alarmToday = _.filter(n, function(o){
				...
				return (fecha >= todayInit && fecha <= todayEndFuture);
			});
			...
			_.forEach(alarmToday, function(v, k){
				...
				if(new Date(v.time) >= new Date() && new Date(v.time) <= todayEnd){
					...
					if(new Date(v.time) <= new Date()){
						$scope.bars[pos].elements.push({
							class: classes[ind]+" grayscale",
							color: color.color
						});
					}

				}
				if(new Date(v.time) >= todayInitFuture && new Date(v.time) <= todayEndFuture){
					$scope.bars[pos].elements.push({
						class: classes[ind]+" bordered",
						color: color.color,
						border: color.color
					});
				}
	...
	.controller('TabAlarmAddCtrl', function($cordovaNetwork, $rootScope, $scope, $cordovaDatePicker, $state, $profile, $toast, $localization, $localStorage, Medicamento, Recordatorio){
		...
		$scope.hexToRgb = function(hex) {
			...
			return result ? {
				r: $rootScope.parseFloat(result[1], 16),
				g: $rootScope.parseFloat(result[2], 16),
				b: $rootScope.parseFloat(result[3], 16)
			} : null;
		};
		...
		$scope.changeKind = function(form){
			form.dosis = null;
			$scope.formaSelected = _.findIndex($rootScope.medicineForma, {nombre: form.kind});
			//form.quantity = null;
		};
		...
		$scope.$watch('alarm', function(n){
			$scope.formaSelected = _.findIndex($rootScope.medicineForma, {nombre: n.kind});
		}, true);
	...
	.controller('TabAlarmEditCtrl', function($cordovaNetwork, $rootScope, $scope, $toast, $timeout, $state, $localization, $stateParams, $localStorage, $cordovaDialogs, Medicamento, Recordatorio){
		...
		$scope.$watch('alarm', function(n){
			$scope.formaSelected = _.findIndex($rootScope.medicineForma, {nombre: n.tipo});
		}, true);
